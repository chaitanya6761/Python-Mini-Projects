...
class RectangularRoom(object):
    ...
    def getRandomPosition(self):
        ...
        pos=Position(random.choice(w),random.choice(h))
        return pos
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for num in range(num_trials):
        ...
        roomObj=RectangularRoom(width,height)
        
        for num in range(num_robots):
            
            bot=robot_type(roomObj,speed)
            botList.append(bot)
            
        while float(roomObj.getNumCleanedTiles())/float(roomObj.getNumTiles()) <=1 :
                
             for bot in botList:
                 anim.update(roomObj, botList)
    ...
    return float(sum(numSteps))/len(numSteps)   
